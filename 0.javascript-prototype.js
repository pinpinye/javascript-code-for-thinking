// 1. constructor不是实例的属性，当你创建函数时，JS会为这个函数自动添加prototype属性，值是一个有 constructor 属性的对象，不是空对象。
// 一旦原型prototype被改写，constructor也会被改写
// 2. instanceof 不是判断右边的参数是不是左边参数的constructor属性，而是判断右边的参数的原型是否存在在左边参数的原型链上
// 只要用这个操作符来测试实例(instance)与原型链中出现过的构造函数,结果就会返回true. 准确的来说这是用来判断对象和函数原型之间的关系
...
// Obeject.create相当于创建了一个新的原型对象，当第二个参数为空的时候，原型相当于目标对象的深拷贝。而实例没有创建新的原型，而是对构造函数原型的引用
...
// 5.Object.prototype 的 __proto__  属性是一个访问器属性（一个getter函数和一个setter函数）, 暴露了通过它访问的对象的内部[[Prototype]] (一个对象或 null)。

// 6.当执行 var a1 = new A(); 时，JavaScript（在内存中创建对象之后，和在运行函数 A() 把 this 指向对象之前）
